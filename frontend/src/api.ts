...
export async function fetchDashboardData(): Promise<DashboardData> {
  ...
  const [overviewData, topRankData, bottomRankData, siteRank, heatmap] = await Promise.all([
    ...
    fetchJson<RankItem[]>(`/api/dashboard/franchises/rank?${params}&metric=total_contribution&direction=desc&limit=30`),
    fetchJson<RankItem[]>(`/api/dashboard/franchises/rank?${params}&metric=total_contribution&direction=asc&limit=12`),
